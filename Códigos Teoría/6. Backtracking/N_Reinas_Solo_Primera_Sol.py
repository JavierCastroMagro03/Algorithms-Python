...
def n_queens(sol, fila_act, N, solucion_encontrada):  # Método n-reinas
    if esSolucion(sol, fila_act):  # Caso base es ver si la solución en la que estoy y la fila por la que voy
        solucion_encontrada = True  # En este caso cambiamos el parámetro
        sol = sol.copy()  # Y nos copiamos la solución
    else:  # Si no (si no he llegado a una solución)
        col = 0  # Voy a mirar todas las columnas
        while not solucion_encontrada and col < N:  # Mientras no haya encontrado una solución
            if esFactible(sol, fila_act, col):  # Si es factible en mi solución actual colocar en una fila y en la columna actual
                ...
                sol, solucion_encontrada = n_queens(sol, fila_act + 1, N, solucion_encontrada)  # Backtraking (seguimos explorando)
    ...
    return sol.copy(), solucion_encontrada  # Devolvemos los dos valores de la llamada recursiva. Necesitamos hacer una copia para recuperar el estado de la lista antes de la llamada recursiva
...
if __name__ == '__main__':
    ...
    sol = n_queens(sol, 0, N,False)  # Para este método necesitaré mi vector, cuál es la fila en la que voy a empezar(0), cuántas reinas tengo que colocar y el último parámetro indica si he encontrado una solución o no
