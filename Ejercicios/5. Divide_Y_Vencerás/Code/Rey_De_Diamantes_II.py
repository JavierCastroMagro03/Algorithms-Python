def busquedabinaria(v, elem, i, j):  # Vector que necesitamos, elemento que buscamos, índice inicial, índice final
    if i > j:  # primero miramos si inicio es mayor que fin
        ...
        if v[k] == elem:  # Si el elemento de la mitad es igual que el que buscamos (elem)
            ...
            return busquedabinaria(v, elem, k + 1, j)  # Buscamos en el lado derecho del array (desplazamos i a esta parte). Importante poner el return al hacer la llamada recursiva
        else:  # Si no, estaré en el caso contrario y lo que tendré que hacer es:
            return busquedabinaria(v, elem, i, k - 1)  # Mover la j hacia la izda (para mirar el lado izquierdo del array). Comprobaremos que el elemento que estamos mirando ahora, es mayor.
